            ...
            switch ($rel['Type']) {
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme':
                    ...
                    if (is_object($xmlTheme)) {
                        ...
                        self::$theme = new Theme($themeName, $colourSchemeName, $themeColours);
                    }
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel['Type']) {
                ...
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    ...
                    foreach ($relsWorkbook->Relationship as $ele) {
                        switch ($ele['Type']) {
                            ...
                            case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotCacheDefinition':
                                ...
                                $excel->addPivotCacheDefinition( (string)$ele['Id'], "$dir/{$ele['Target']}", $this->securityScan($this->getFromZipArchive($zip, "$dir/{$ele['Target']}" ) ) );
                                break;
                        }
                    }
					...
					if ($xmlWorkbook->pivotCaches )
					{
						foreach ( $xmlWorkbook->pivotCaches->children() as $elementName => /** @var \SimpleXMLElement $pivotCache */ $pivotCache)
						{
							$cacheId = (string)$pivotCache['cacheId'];
							$rId = (string)$pivotCache->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships')->id;
							$excel->pivotCacheDefinitionCollection->addPivotCacheIndex( $cacheId, $rId );
						}
					}
                            ...
                            if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                ...
                                foreach ($relsWorksheet->Relationship as $ele) {
									...
									else if ($ele['Type'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/pivotTable')
									{
										...
										$cacheId = (string)$pivotTableXml->attributes()->cacheId;

										$pivotTable = $excel->addPivotTable( (string)$ele['Id'], $path, $xml, $cacheId, "$dir/$fileWorksheet" );

										$rId = $excel->pivotCacheDefinitionCollection->getPivotCacheIndex( $cacheId );
										$path = $excel->pivotCacheDefinitionCollection->getPivotCacheDefinitionPath( $rId );
										$pivotTable->cacheDefinitionPath = $path;

										// Note: could also get to the cache by looking in the _rels/pivotTable[x].xml.rels file
		                                $relsPivotCache = simplexml_load_string(
		                                    $this->securityScan(
		                                        $this->getFromZipArchive($zip, dirname( $path ) . '/_rels/' . basename( $path ) . '.rels')
		                                    ),
		                                    'SimpleXMLElement',
		                                    Settings::getLibXmlLoaderOptions()
		                                );
