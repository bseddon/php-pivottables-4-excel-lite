...
class Workbook extends WriterPart
{
    ...
    public function writeWorkbook(Spreadsheet $spreadsheet, $recalcRequired = false)
    {
		...
		// pivot table caches.  This MUST come last except for the <extLst>
		// tag (if used) or the workbook is regarded as being invalid.
     ...
     * @throws WriterException
     */
    private function writePivotCaches(XMLWriter $objWriter, Spreadsheet $spreadsheet)
    {
		// Write caches
		if ( ! $spreadsheet->pivotTables->hasPivotTables() ) return;

		$objWriter->startElement('pivotCaches');

		foreach ( $spreadsheet->pivotTables as $path => /** @var PivotTable $table */ $table )
		{
			$objWriter->startElement('pivotCache');
			$objWriter->writeAttribute( 'cacheId', $table->cacheId );
			$objWriter->writeAttribute( 'r:id', $table->afterSaveReferenceId );
			$objWriter->endElement();
		}
